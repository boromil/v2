...
| D7 | **Timezone ignored.** `elapsedTime` uses `time.Since` (server TZ semantics) and entry content header formats raw `Date` in server-local; classic converts via `timezone.Convert(tz, t)` and `elapsed` takes `$.user.Timezone`. | `handler.go:1299` vs `functions.go:151,284` | Medium |

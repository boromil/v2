...
- **Squished reading pane (fixed)**: default grid was `240px 1fr 4px 1fr`, splitting list and article 50/50 (content only ~598px at 1440px). Now `240px minmax(320px, 420px) 4px 1fr` — list capped at 420px, article pane takes the rest (776px at 1440px), matching Google Reader's wide reading pane. Resize handle (writes `240px {w}px 4px 1fr`) stays compatible.
